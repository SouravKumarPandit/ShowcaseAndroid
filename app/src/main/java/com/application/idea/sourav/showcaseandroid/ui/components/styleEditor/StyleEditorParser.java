...
public class StyleEditorParser
{
...
/*
        ...
        for (int i = 0; i < text.length(); i = next) {
            next = text.nextSpanTransition(i, text.length(), BackgroundColorSpan.class);
            BackgroundColorSpan[] styles = text.getSpans(i, next, BackgroundColorSpan.class);
//            if (styles.length > 0) {
//                withinBackGround(out, text, i, next++, Color.GREEN);
//            } else {
...
//todo handled bullet point and quote above
        withinContent(out, text, 0, text.length());


    }

    private static void withinAbsoluteSize(StringBuilder out, Spanned text, int start, int end, int green) {
    ...
    private static void withBgContent(StringBuilder out, Spanned text, int i, int next) {
    }
        ...
        for (int i = start; i < end; i = next) {
            ...
            for (int j = 0; j < spans.length; j++) {
                ...
                if (spans[j] instanceof AbsoluteSizeSpan) {
                    int textSize = ((AbsoluteSizeSpan) spans[j]).getSize();
                    out.append("<span style=\"font-size:").append(textSize).append("px\";>");
                }
                if (spans[j] instanceof BackgroundColorSpan) {
                    int bgColor = ((BackgroundColorSpan) spans[j]).getBackgroundColor();
                    String hexColor = Integer.toHexString(bgColor).substring(2);
                    out.append("<span style=\"background-color:#").append(hexColor).append(";\">");
                }
                if (spans[j] instanceof ForegroundColorSpan) {
                    int fgColor = ((ForegroundColorSpan) spans[j]).getForegroundColor();
                    String hexColor = Integer.toHexString(fgColor).substring(2);
                    out.append("<span style=\"color:#").append(hexColor).append(";\">");
                }
